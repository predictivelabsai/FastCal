...
def partner_section():
    return Section(
        Span("Partners", cls="eyebrow"), H2("Connect with trusted integration specialists."),
        P("Identity, software delivery, data engineering and applied-AI expertise for FastSME implementations."),
        Div(*[A(Img(src=logo, alt=f"{name} logo", loading="lazy"), Small("Integration Partner"), H3(name), P(description), href=url, target="_blank", rel="noopener noreferrer", cls="partner-card") for name, url, logo, description in PARTNERS], cls="partner-grid"),
        id="partners", cls="partners",
    )


def landing():
    return Html(Head(Title("FastCal — Open team calendar"), Meta(name="viewport", content="width=device-width, initial-scale=1"), Style(CSS)),
        Body(Nav(A("Fast", B("Cal"), href="/", cls="brand"), A("Partners", href="#partners"), A("Developers", href="/developers"), A("Sign In", href="/auth/suite", cls="signin"), cls="nav"),
            Main(Section(Div(Span("OPEN TEAM CALENDAR", cls="eyebrow"), H1("Make time work for everyone."),
                P("Coordinate calendars, availability, recurring events, reminders and FastMeet links in one open workspace."),
                A("Open FastCal", href="/auth/suite", cls="cta")),
                Div(Div(*[Div(Strong(day), Div("Team planning", cls="event") if day == "Tue" else None, cls="day") for day in ("Mon","Tue","Wed","Thu","Fri")], cls="week"), cls="preview"), cls="hero"),
                Section(Div(Article(H3("Shared calendars"), P("Keep team schedules visible without giving up ownership.")),
                    Article(H3("Availability and reminders"), P("Coordinate across time zones and keep commitments on track.")),
                    Article(H3("FastMeet connected"), P("Attach meeting rooms and carry scheduling into follow-through."))), cls="features"),
                partner_section())))


def shell(identity: dict, content):
    return Html(Head(Title("FastCal"), Meta(name="viewport", content="width=device-width, initial-scale=1"), Style(CSS)),
        Body(Div(Aside(A("Fast", B("Cal"), href="/", cls="brand"), A("+ New event", href="/events/new", cls="new"),
            P(identity["org_name"]), Small(identity["email"]), P(A("FastOffice", href="/launch/office")), P(A("Sign out", href="/logout")), cls="side"),
            Main(content, cls="main"), cls="shell")))


def home(identity: dict, rows: list[dict]):
    items = [Article(Div(Strong(r["starts_at"][0:10]), Br(), Small(r["starts_at"][11:16])),
        Div(H3(r["title"]), P(" · ".join(x for x in (r["calendar_name"], r["location"], r["attendees"]) if x))),
        Form(Input(type="hidden", name="event_id", value=r["id"]), Button("Delete", cls="btn"), method="post", action="/events/delete"),
        cls="event-row") for r in rows]
    return shell(identity, (Div(Div(Span("CALENDAR", cls="eyebrow"), H1("Upcoming events")), A("Search", href="/search"), cls="top"), Div(*(items or [P("No events yet.")]), cls="events")))


def event_form(identity: dict, calendars: list[dict]):
    return shell(identity, (Span("NEW EVENT", cls="eyebrow"), H1("Schedule time"),
        Form(Label("Title", Input(name="title", required=True)), Label("Calendar", Select(*[Option(c["name"], value=c["id"]) for c in calendars], name="calendar_id")),
            Label("Starts", Input(type="datetime-local", name="starts_at", required=True)), Label("Ends", Input(type="datetime-local", name="ends_at", required=True)),
            Label("Time zone", Input(name="timezone", value="Europe/Tallinn")), Label("Reminder (minutes)", Input(type="number", name="reminder_minutes", value="15")),
            Label("Location", Input(name="location")), Label("FastMeet URL", Input(type="url", name="meet_url")),
            Label("Attendees", Input(name="attendees", placeholder="name@example.com, …"), cls="wide"),
            Label("Recurrence", Input(name="recurrence", placeholder="RRULE:FREQ=WEEKLY"), cls="wide"),
            Label("Description", Textarea(name="description", rows="4"), cls="wide"), Button("Create event", cls="btn wide"), method="post", action="/events", cls="card")))


def booking_pages(identity: dict, rows: list[dict]):
    cards = [Article(H3(row["title"]), P(f"{row['duration_minutes']} minutes · {row['timezone']}"),
        A("Open public page", href=f"/book/{row['slug']}", target="_blank"),
        cls="event-row") for row in rows]
    return shell(identity, (Div(Span("SCHEDULING", cls="eyebrow"), H1("Booking pages"),
        P("Let customers and outside partners reserve available time without an account.")),
        Div(*cards, cls="events"),
        H2("Create booking page"),
        Form(Label("Meeting name", Input(name="title", required=True)),
            Label("Duration", Select(*[Option(f"{x} minutes", value=x) for x in (15, 30, 45, 60)], name="duration_minutes")),
            Label("Available from", Input(type="time", name="available_from", value="09:00")),
            Label("Available to", Input(type="time", name="available_to", value="17:00")),
            Label("Time zone", Input(name="timezone", value="Europe/Tallinn")),
            Label("Minimum notice hours", Input(type="number", name="minimum_notice_hours", value="4")),
            Label("Buffer minutes", Input(type="number", name="buffer_minutes", value="10")),
            Label("Location or FastMeet link", Input(name="location")),
            Label("Description", Textarea(name="description", rows="3"), cls="wide"),
            Button("Publish booking page", cls="btn wide"), method="post", action="/booking-pages", cls="card")))


def public_booking(page: dict, slots: list[dict], message: str = ""):
    return Html(Head(Title(f"Book {page['title']} · FastCal"), Meta(name="viewport", content="width=device-width, initial-scale=1"), Style(CSS)),
        Body(Nav(A("Fast", B("Cal"), href="/", cls="brand"), cls="nav"),
            Main(Section(Div(Span(page["organisation_name"].upper(), cls="eyebrow"), H1(page["title"]),
                P(page["description"] or f"Choose a time with {page['owner_name']}."),
                P(f"{page['duration_minutes']} minutes · {page['timezone']}"), P(message) if message else None),
                Form(Label("Available time", Select(*[Option(s["label"], value=s["starts_at"], data_end=s["ends_at"]) for s in slots], name="starts_at", id="slot")),
                    Input(type="hidden", name="ends_at", id="slot-end", value=slots[0]["ends_at"] if slots else ""),
                    Label("Your name", Input(name="guest_name", required=True)), Label("Your email", Input(type="email", name="guest_email", required=True)),
                    Label("Anything we should know?", Textarea(name="guest_notes", rows="3"), cls="wide"),
                    Button("Confirm booking", cls="btn wide", disabled=not slots),
                    Script("document.getElementById('slot')?.addEventListener('change',e=>document.getElementById('slot-end').value=e.target.selectedOptions[0].dataset.end)"),
                    method="post", action=f"/book/{page['slug']}", cls="card"), cls="hero"))))
